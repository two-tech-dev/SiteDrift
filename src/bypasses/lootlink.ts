...
export default class LootLink extends BypassDefinition {
  ...
  execute() {
    if (/[?&]r=/.test(window.location.href.toString())) {
      ...
      // https://loot-link.com/s?fJjn
      const originalFetch = window.fetch;
      window.fetch = (url, config) =>
        this.customFetch(url, config, originalFetch);
    }
  }
    ...
    return response
      ...
      .then((data) => {
        ...
        navigator.sendBeacon(
          `https://${
            parseInt(urid.substr(-5), 10) % 3
          }.${INCENTIVE_SERVER_DOMAIN}/st?uid=${urid}&cat=${task_id}`
        );
        fetch(
          `https://${INCENTIVE_SYNCER_DOMAIN}/td?ac=1&urid=${urid}&&cat=${task_id}&tid=${TID}`
        );
  ...
  setupWebSocket(urid, task_id) {
    const ws = new WebSocket(
      `wss://${
        parseInt(urid.substr(-5), 10) % 3
      }.${INCENTIVE_SERVER_DOMAIN}/c?uid=${urid}&cat=${task_id}&key=${KEY}`
    );
    ...
    ws.onmessage = (event) => {
      if (event.data.includes('r:')) {
        this.helpers.safelyNavigate(
          this.decryptData(event.data.replace('r:', ''))
        );
      }
    };
  }
